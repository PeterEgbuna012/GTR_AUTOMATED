Feature: 0.0.5 - Completing an EM Work Order for Review

You have given an EM work order and now will complete it.
...
Scenario: 01) Create a EM Work Order
    ...
    And I enter following data:
      | Summary | Details |
      | AUTOT COMPLETE EM WO TEST | AUTO TEST EM WORK ORDER |
    ...
    And I take screenshot of Completing an EM Work Order for Review scenario one
...
Scenario:  02) Accepting An EM Work Order into the Work Bank
     ...
     When I filter table EM:
		...
		| AUTOT COMPLETE EM WO TEST | =WAIT-REV | =EM |
...
Scenario: 03)  Assigning an EM Work Orderr
	Given I sign as teamlead
      ...
      When I filter table EM:
		| Description | Status | Work Type |
		| AUTOT COMPLETE EM WO TEST | =REVIEWED | =EM |
     And I select no. 1 record from EM table
...
Scenario: 04) Completing an EM Work Order for Review
    ...
    And I open application Quick Work Order (BRDA)
    When I filter table Quick Work Order:
		| Description | Status | Work Type |
		| AUTOT COMPLETE EM WO TEST | =SCHEDULED | =EM |
    And I select no. 1 record from EM table
	And I press route the workflow button
	...
	And I set (Radio) field to "Complete Work Assignment"
    And I press dialog Ok
	And I press dialog Ok button of system message
    #And I press dialog button close
    ...
    And I enter following:
      | Summary       | Details |
      | AUTOT COMPLETE EM WO TEST | COMPLETE EM WORK ORDER AUTO TEST |
    And I save
    #And I press route the workflow button
    And I press route the workflow button
    And I set (Radio) field to "Complete Work Assignment"
