Feature: 0.0.6 - Rejecting an EM Work Order

You are reviewing Completed Work in your start centre and want to reject it to be reviewed by the person who completed the work


@Feature:0.0.6-RejectinganEMWorkOrder
Scenario: 01) Create a EM Work Order
    ...
    And I enter following data:
      | Summary | Details |
      | Rejecting an EM Work Order | AUTO TEST EM WORK ORDER |
    ...
    And I take screenshot of Rejecting an EM Work Order scenario one
...
Scenario:  02) Accepting An EM Work Order into the Work Bank
     ...
     When I filter table EM:
		...
		| Rejecting an EM Work Order | =WAIT-REV | =EM |
	...
	And I take screenshot of Rejecting an EM Work Order scenario two
...
Scenario: 03)  Assigning an EM Work Orderr
	Given I sign as <role>
      ...
      When I filter table EM:
		| Description | Status | Work Type |
		| Rejecting an EM Work Order | =REVIEWED | =EM |
     And I select no. 1 record from EM table
	 ...
	 And I take screenshot of Rejecting an EM Work Order scenario three
...
Scenario: 04) Completing an EM Work Order for Review
    ...
    When I filter table Quick Work Order:
		| Description | Status | Work Type |
		| Rejecting an EM Work Order | =SCHEDULED | =EM |
    And I select no. 1 record from EM table
	And I press route the workflow button
	And I set (Radio) field to "Start Work"
    ...
    And I enter following:
      | Summary       | Details |
      | Rejecting an EM Work Order | COMPLETE EM WORK ORDER AUTO TEST |
    And I save
    And I press route the workflow button
    And I press route the workflow button
    And I set (Radio) field to "Complete Work Assignment"
    ...
    And I take screenshot of Rejecting an EM Work Order test scenario four
...
Scenario: 05) Rejecting an EM Work Order
	Given I sign as <role>
	And I open application Work Order Tracking BRDA from Fav Menu
    When I filter table EM:
		| Description | Status | Work Type |
		| Rejecting an EM Work Order | =COMP-WREV | =EM |
    And I select no. 1 record from EM table
	And I press route the workflow button
	And I set (Radio) field to "Reject this Work Order"
	And I press OK button
	And I Status field has value INPRG-HOLD
	And I take screenshot of Rejecting an EM Work Order test scenario five
