Feature: 3.0.4 - Approve a PM Work Order Task
...
Scenario:01) Create a PM
    ...
    And I take screenshot of Approve a PM Work Order Task scenario one
  ...
  Scenario:02) Generating Ad Hoc PM Work Order
    ...
    When I filter table PMs:
      ...
      | AUTO APPROVE A PM WO TEST | =ACTIVE | ~Null~ |
    ...
    And I take screenshot of Approve a PM Work Order Task Scenario two
    And I press the button Ok
...
Scenario: 03) Assigning a PM Work Order Task
  ...
  When I filter table PM:
		...
		| AUTO APPROVE A PM WO TEST | =SCHEDULED | =M1 |
  And I select no. 1 record from PMs table
  ...
  And I open Select Value lookup for Labor field
   And I filter Labor table:
      | Labor |
      | ISCOTT | 
  And I select no. 2 from Labor table records
  And I save record
  And I take screenshot of Approve a PM Work Order Task Scenario three
...
Scenario: 04) Complete a PM Work Order Task
  ...
  When I filter table PM:
		| Description | Status | Work Type |
		| AUTO APPROVE A PM WO TEST | =INPRG | =M1 |
  And I select no. 1 record from PMs table
  And I press route the workflow button
  ...
  And I filter table Labor:
      | Labor |
      | ISCOTT |
  And I select no. 5 record from Labor table records
  And I set Start and End time
  And I save record
  And I press route the workflow button
  Then Status field has value COMP-WREV
  And I take screenshot of Approve a PM Work Order Task scenario four
...
Scenario: 05) Approve a PM Work Order Task
  Given I sign as maintainer teamlead 
  And I open application Work Order Tracking BRDA from Favorite Menu
  When I filter table PM:
		| Description | Status | Work Type |
		| AUTO APPROVE A PM WO TEST | =COMP-WREV | =M1 |
  And I select no. 1 record from PMs table
  And I press route the workflow button
  And I set (Radio) field Approve Work
  And I press OK dialog button
  And Status field has value COMP
  And I take screenshot of Approve a PM Work Order Task test scenario five
